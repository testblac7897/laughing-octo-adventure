...
def check_password():
    """Einfache Passwortprüfung für den Chat Viewer."""
    # Standardpasswort direkt als String (für diesen Testfall einfacher)
    # In einer Produktionsumgebung sollte ein sicherer Hash verwendet werden
    correct_password = "chatviewer123"
    ...
    if password:
        if password == correct_password:
            ...
            st.error(f"❌ Falsches Passwort. Bitte versuche es erneut.")
            return False
...
if check_password():
    ...
    *Das Standardpasswort ist "chatviewer123".*
    ...
    with st.expander("Passwort ändern"):
        st.markdown("""
        Um das Passwort zu ändern, öffne die Datei h5-chat-viewer.py und suche diese Zeile:
        
        ```python
        correct_password = "chatviewer123"
        ```
        
        Ändere "chatviewer123" in dein gewünschtes Passwort.
        """)